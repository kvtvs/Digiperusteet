...
const hae = function(event ){
    ...
    $.getJSON(apiURL + $(this).serialize(), function(response){
        ...
        $.each(response, function(indeksi, sarja){
            ...
            $('#tulos').append(`<article class="col d-flex flex-column align-items-center mt-2 mb-5">
            <h2>${sarja.show.name}</h2>
            <p>${sarja.show.genres}</p>
            <a href="${sarja.show.officialSite ? sarja.show.officialSite : sarja.show.url}" class="mb-1"><i>Linkki kotisivulle</i></a>
            <figure>
                <img src="${sarja.show.image ? sarja.show.image.medium : 'img/tile.png'}" alt="${sarja.show.name}">
